...
class TransitTourSkim(Skim):
    ...
    def getTourAttributes(self, orig_taz, dest_taz, tour_type):
        """
        Returns a dict of all transit attributes listed in :py:attr:`TransitTourSkim.TABLE_NUMBER_TO_NAME`
        ...
        for tablenum,tablename in TransitTourSkim.TABLE_NUMBER_TO_NAME.items():
            ...
            if tablename in TransitTourSkim.TIME_SKIMS:
                transitAttributes[tablename] = 0.01 * self.skim_table_files[skim_file].root._f_get_child("%d" % tablenum)[orig_taz-1][dest_taz-1]
            ...
            # FAREs are in the correct units already
                ...
                transitAttributes[tablename] = self.skim_table_files[skim_file].root._f_get_child("%d" % tablenum)[orig_taz-1][dest_taz-1]
